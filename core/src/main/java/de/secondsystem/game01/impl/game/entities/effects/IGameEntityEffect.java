...
public interface IGameEntityEffect extends ISerializable, IUpdateable, IInsideCheck {
	...
	void draw(RenderTarget rt, WorldId worldId, Vector2f position, float rotation, int worldMask);
...
interface IGameEntityEffectFactory {
	IGameEntityEffect create(IGameMap map, Attributes attributes, int worldMask, Vector2f position, float rotation, IWorldDrawable representation);
}
